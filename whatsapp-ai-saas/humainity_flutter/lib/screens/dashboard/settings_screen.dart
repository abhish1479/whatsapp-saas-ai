...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text('Manage your business details and configurations.', style: TextStyle(color: AppColors.mutedForeground)),
  ...
  Widget _buildBusinessInfo(BusinessProfileState state) {
    if (state.isLoading) {
      return const AppCard(child: SizedBox(height: 200, child: Center(child: CircularProgressIndicator())));
    }
        ...
        children: [
          const Row(
            children: [
              ...
              SizedBox(width: 8),
              Text('Business Information', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
            ],
          ),
          const SizedBox(height: 24),
          ...
          SizedBox(
            ...
            child: AppButton(
              ...
              onPressed: _isSavingProfile ? null : () => _updateProfile(state.profile?.id),
            ),
          ),
        ],
      ),
  ...
  Future<void> _updateProfile(int? id) async {
    ...
    try {
      ...
      await ref.read(businessProfileProvider.notifier).updateProfile(updatePayload);
      ...
      child: Column(
        children: [
          Row(
            ...
            children: [
              const Row(children: [Icon(LucideIcons.creditCard, color: AppColors.primary, size: 20), SizedBox(width: 8), Text('Billing & Payment', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600))]),
              ToggleSwitch(
                minWidth: 60.0, initialLabelIndex: _currency == 'INR' ? 0 : 1, cornerRadius: 8.0,
                activeFgColor: AppColors.foreground, inactiveBgColor: AppColors.muted, inactiveFgColor: AppColors.mutedForeground,
                activeBgColor: const [AppColors.background], totalSwitches: 2, labels: const ['INR', 'USD'],
                onToggle: (index) => setState(() => _currency = (index == 0 ? 'INR' : 'USD')),
              ),
            ],
          ),
          const SizedBox(height: 24), const Divider(height: 48), AppButton(text: 'Recharge Wallet', onPressed: () {}),
        ],
      ),
    );
  }
  ...
  Widget _buildAiModelConfig() {
    return AppCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Row(children: [Icon(LucideIcons.bot, color: AppColors.primary, size: 20), SizedBox(width: 8), Text('AI Model Configuration', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600))]),
          const SizedBox(height: 24),
          AppDropdown<String>(
            labelText: 'LLM Model', value: _llmModel,
            onChanged: (val) => setState(() => _llmModel = val!),
            items: const [DropdownMenuItem(value: 'google/gemini-2.5-flash', child: Text('Gemini 2.5 Flash'))],
          ),
          const SizedBox(height: 16),
          AppDropdown<String>(
            labelText: 'Voice', value: _selectedVoice,
            onChanged: (val) => setState(() => _selectedVoice = val!),
            items: const [DropdownMenuItem(value: 'Sarah', child: Text('Sarah'))],
          ),
          const SizedBox(height: 24), AppButton(text: 'Save Model Settings', onPressed: () {}),
        ],
      ),
    );
  }
  ...
  Widget _buildNotifications() {
    return AppCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Row(children: [Icon(LucideIcons.bell, color: AppColors.primary, size: 20), SizedBox(width: 8), Text('Notifications', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600))]),
          const SizedBox(height: 24),
          _buildSwitchTile(title: 'Email Notifications', value: _emailNotifications, onChanged: (val) => setState(() => _emailNotifications = val)),
          const Divider(height: 24),
          _buildSwitchTile(title: 'Campaign Alerts', value: _campaignAlerts, onChanged: (val) => setState(() => _campaignAlerts = val)),
        ],
      ),
    );
  }
  ...
  Widget _buildSecurity() {
    return AppCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Row(children: [Icon(LucideIcons.shield, color: AppColors.primary, size: 20), SizedBox(width: 8), Text('Security', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600))]),
          const SizedBox(height: 24),
  ...
  Widget _buildSwitchTile({required String title, required bool value, required ValueChanged<bool> onChanged}) {
    return Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(title, style: const TextStyle(fontWeight: FontWeight.w500)), AppSwitch(value: value, onChanged: onChanged)]);
  }
}