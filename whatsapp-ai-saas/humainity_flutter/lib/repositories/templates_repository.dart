...
class TemplatesRepository {
  ...
  Future<List<Template>> getTemplates(String tenantId) async {
    final uri =
        Uri.parse('$_baseUrl/templates/get_templates_list?tenant_id=$tenantId');
  ...
  Future<Template> createTemplate(
      String tenantId, Map<String, dynamic> templateData) async {
    ...
    try {
      ...
      if (response.statusCode == 200 && responseBody['success'] == true) {
        ...
        throw Exception(responseBody['error']?['details']?.join(', ') ??
  ...
  Future<Template> updateTemplate(
      int templateId, Map<String, dynamic> templateData) async {
    ...
    try {
      ...
      if (response.statusCode != 200 || responseBody['success'] != true) {
        ...
        if (responseBody['error']?['code'] == 'DELETE_RESTRICTED') {
          throw Exception(
              responseBody['message'] ?? 'Cannot delete activated template');
        }
        throw Exception(responseBody['error']?['details']?.join(', ') ??
